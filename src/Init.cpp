...
double inthelperf_fluxtube_z(double z, void *p) {
    ...
    /*cout << "Quarks: " << endl;
    cout << par->quarks[0] << endl << par->quarks[1] << endl << par->quarks[2]
    << endl; cout << "center " << par->center << endl; cout << "b " << b3d <<
    endl;
    */
    ...
    for (unsigned int i = 0; i < par->quarks.size(); i++) {
        ...
        if (quark_to_b.LenSqr() > quark_to_center.LenSqr()
            ...
            // cout << "distance from quark "<< i << ": " << quark_to_b.Len() <<
            // "   quark_to_center dist " << quark_to_center.Len() << endl;
        ...
        for (int ipos = 0; ipos < N * N; ipos++) {
            ...
            for (int i = 0; i < A1; i++) {
                ...
                if (param->getUseConstituentQuarkProton() > 0) {
                    ...
                    Vec b_nucleon =
                        Vec((xm - x), (ym - y),
                            0);  // distance from the center of the nucleon

                    T = FluxTubeThickness(hotspots, Qsflucts, b_nucleon, param);
                } else {
                    messager.error(
                        "Stringy proton requires constituent quarks.");
                    exit(1);
                }
